 ...
 * segment plus the leaf ("shadow"/"telemetry"), and Content-Format (JSON,
 * only when a payload will follow). No credential rides in the message:
 * the PSK handshake authenticates the device, and the platform maps the
 * handshake identity (the pigeon's id) to the pigeon, rejecting any
 * request whose Uri-Path names a different one. Works on any coap_packet
 * regardless of how its header was framed (RFC 7252 or 8323).
 */
